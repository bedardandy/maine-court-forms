...
def process(kv_map: dict, case: dict) -> tuple[dict, list]:
    ...
    # Self-identification: "I am the [petitioner / defendant]"
    self_role = facts.get("fm_self_role", "petitioner")  # or "defendant"
    ...
    # Other-party residence (town/county)
    other = defendant if self_role.lower() == "petitioner" else plaintiff
    other_town = other.get("city", facts.get("other_party_town", "Saco"))
    other_county = court.get("county", "Cumberland")
    ...
    # Paragraph 3 checkboxes — typical default: select "A" (no special relief)
    for fid in ("paragraph_3_checkboxes_a_b_c_d_e",
                 "paragraph_3_a", "paragraph_3"):
        if _set(out, fid, "A"):
            changes.append((fid, "A", "para3-default-A"))

    # Magistrate-order objection narrative (FM-071) — schema field_id
    # is the long form ending in "final_order"
    objection = facts.get("magistrate_objection",
                             "I object to the entry of the Magistrate's Final Order "
                             "because the calculation of child support did not "
                             "include the recent change in my employment income.")
    for fid in (
    ...
    modify = facts.get("magistrate_modify",
                        "Modify Item 3 (child support) to reflect the "
                        "recent change in income.")
    if _set(out, "modify_it_as_follows", modify):
        changes.append(("modify_it_as_follows", modify, "modify"))
    request = facts.get("magistrate_request",
                          "Therefore, I request that a Judge review the "
                          "Magistrate's Final Order, hold a hearing, and "
                          "enter such order as the Court deems just.")
    if _set(out, "therefore_i_request_that_a_judge_1", request):
        changes.append(("therefore_i_request_that_a_judge_1", request,
                        "request"))
    # FM-071 acknowledgment checkboxes (always Yes)
    for fid in (
    ...
    # variant was wrong; widget has no suffix on FM-188).
    property_text = facts.get("fm_property_credits",
                                "marital residence at 142 Pine Street, "
                                "Portland; joint Bank of America checking "
                                "and savings accounts; and one (1) 2019 "
                                "Toyota Camry registered jointly")
    ...
    # FM-188 "will_not_be_affected" narrative
    affected_narr = facts.get("fm_will_not_be_affected",
        "the marital residence and personal effects of the parties")
    ...
    # FM-214 — emergency-order specifics
    if _set(out,
    ...
    # FM-214 "I now reside in town" split widgets
    pl_city = plaintiff.get("city", "")
    pl_state = plaintiff.get("state", "ME")
    cur_county = court.get("county", "Cumberland")
    if _set(out,
    ...
    if _set(out, "county_1", cur_county):
        changes.append(("county_1", cur_county, "county1"))
    if _set(out, "state_1", pl_state):
        changes.append(("state_1", pl_state, "state1"))

    # "The other party now resides in town"
    if _set(out, "the_other_party_now_resides_in_town", other_town):
        changes.append(("the_other_party_now_resides_in_town", other_town,
                        "other-town-narr"))
    if _set(out, "town_2", other_town):
        changes.append(("town_2", other_town, "town2"))
    if _set(out, "county_2", other_county):
        changes.append(("county_2", other_county, "county2"))
    if _set(out, "state_2", "ME"):
        changes.append(("state_2", "ME", "state2"))

    # FM-214 custody determination block
    custody_court = facts.get("fm_custody_court",
        f"Cumberland County Probate Court, Portland, Maine")
    if _set(out,
    ...
    # FM-214 — resident-of-Maine radio (filer IS Maine resident by default)
    if _set(out, "i_am_a_resident_of_the_state_of_maine", "X"):
        changes.append(("i_am_a_resident_of_the_state_of_maine", "X",
                        "fm214-resident"))

    # FM-214 — order more specifically (1+2 line narrative)
    order_more = facts.get("fm214_order_more_specifically", [
        "The defendant has failed to comply with the visitation provisions "
        "of the Judgment dated above, denying the plaintiff scheduled "
        "weekend visitation on two consecutive weekends.",
        "The defendant has refused to respond to the plaintiff's written "
        "requests for make-up visitation.",
    ])
    ...
    # FM-214 — "I have provided at least two days notice" radio (default Yes)
    if _set(out, "i_have_provided_at_least_two_days_notice_to_the_custodial_parent_or", "X"):
